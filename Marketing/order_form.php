...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $checkSql = "SELECT * FROM client_info WHERE client_id = :client_id";
        ...
        if ($existingClient) {
            // Store old created_at value before updating
            $old_created_at = $existingClient['created_at'];

            // Move existing client info to history before updating
            $historySql = "INSERT INTO client_edit_records (user_id, client_id, client_name, email, phone_no, alt_no, whatsapp_no, city, state, address, sale_person, created_at) 
                           VALUES (:user_id, :client_id, :client_name, :email, :phone_no, :alt_no, :whatsapp_no, :city, :state, :address, :sale_person, :created_at)";
            $historyStmt = $pdo->prepare($historySql);
            $historyStmt->bindParam(':user_id', $existingClient['user_id']);
            $historyStmt->bindParam(':client_id', $existingClient['client_id']);
            $historyStmt->bindParam(':client_name', $existingClient['client_name']);
            $historyStmt->bindParam(':email', $existingClient['email']);
            $historyStmt->bindParam(':phone_no', $existingClient['phone_no']);
            $historyStmt->bindParam(':alt_no', $existingClient['alt_no']);
            $historyStmt->bindParam(':whatsapp_no', $existingClient['whatsapp_no']);
            $historyStmt->bindParam(':city', $existingClient['city']);
            $historyStmt->bindParam(':state', $existingClient['state']);
            $historyStmt->bindParam(':address', $existingClient['address']);
            $historyStmt->bindParam(':sale_person', $existingClient['sale_person']);
            $historyStmt->bindParam(':created_at', $old_created_at);
            $historyStmt->execute();

            // Update the latest client info
            $updateSql = "UPDATE client_info SET
                            ...
                            created_at = NOW()  -- Update created_at to current timestamp
                          WHERE client_id = :client_id";
            $updateStmt = $pdo->prepare($updateSql);
            $updateStmt->bindParam(':client_id', $client_id);
            $updateStmt->bindParam(':client_name', $client_name);
            $updateStmt->bindParam(':email', $email);
            $updateStmt->bindParam(':phone_no', $phone_no);
            $updateStmt->bindParam(':alt_no', $alt_no);
            $updateStmt->bindParam(':whatsapp_no', $whatsapp_no);
            $updateStmt->bindParam(':city', $city);
            $updateStmt->bindParam(':state', $state);
            $updateStmt->bindParam(':address', $address);
            $updateStmt->bindParam(':sale_person', $sale_person);
            $updateStmt->execute();
        }
     else {
        // Insert a new client
        $insertSql = "INSERT INTO client_info (client_id, user_id, current_date_time, client_name, email, phone_no, alt_no, whatsapp_no, city, state, address, sale_person) 
        ...
        $insertStmt->bindParam(':client_id', $client_id);
        $insertStmt->bindParam(':user_id', $user_id);
        $insertStmt->bindParam(':current_date_time', $current_date_time);
        $insertStmt->bindParam(':client_name', $client_name);
        $insertStmt->bindParam(':email', $email);
        $insertStmt->bindParam(':phone_no', $phone_no);
        $insertStmt->bindParam(':alt_no', $alt_no);
        $insertStmt->bindParam(':whatsapp_no', $whatsapp_no);
        $insertStmt->bindParam(':city', $city);
        $insertStmt->bindParam(':state', $state);
        $insertStmt->bindParam(':address', $address);
        $insertStmt->bindParam(':sale_person', $sale_person);
        $insertStmt->execute();
    } 
        echo "<script>alert('Customer information saved successfully.'); window.location.href='order_form.php';</script>";
        exit();
...
<body>
    <div class="container mt-5 p-5 pt-0">
        <div class="row g-2 mb-2">
            <h3 class="text-center mb-1"><?php echo "Hi👋, " . $_SESSION["username"];?><?php echo "Hi👋, " . $_SESSION["user_id"];?>
            </h3>
            <div class="col-6">
